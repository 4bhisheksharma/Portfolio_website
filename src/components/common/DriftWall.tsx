...
export interface DriftWallProps<T extends DriftWallItem = DriftWallItem> {
  items?: T[];
  columns?: number;
  tileWidth?: number;
  tileHeight?: number;
...
function DriftWall<T extends DriftWallItem = DriftWallItem>({
    ...
    if (onTileClick) {
      return (
        <button
          key={id}
          type="button"
          aria-label={item.title ?? "View image"}
